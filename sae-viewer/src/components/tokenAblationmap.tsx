...
export default function TokenAblationmap({ info, colors = DEFAULT_COLORS, renderNewlines }: Props) {
  ...
  if (!info.ablate_loss_diff || info.ablate_loss_diff.length === 0) {
    return null;
  }
  ...
  return (
    <div className="block" style={{width:'100%', whiteSpace: 'pre-wrap'}}>
      {info.tokens.map((token, idx) => {
        ...
        const loss_diff = (idx === 0) ? 0 : (info.ablate_loss_diff?.[idx-1] ?? 0);
        const kl = (idx === 0) ? 0 : (info.kl?.[idx-1] ?? 0);
                ...
                <thead>
                  <tr>{[
                    ...
                    <React.Fragment key={i}>
                      <th>Upvoted {suffix}</th><th /><th>Downvoted {suffix}</th><th />
                    </React.Fragment>
                  ))}</tr>
                </thead>
                <tbody>
                  {top_upvotes.map((upvote, j) => {
                    ...
                    return (
                      <tr key={j}><td>{(upvote ?? 0).toExponential(1)}</td><td style={{ whiteSpace: "pre" }}>{top_upvote_tokens[j] ?? ""}</td><td>{(downvote ?? 0).toExponential(1)}</td><td style={{ whiteSpace: "pre" }}>{top_downvote_tokens[j] ?? ""}</td><td>{(top_upvotes_probs[j] ?? 0).toExponential(1)}</td><td style={{ whiteSpace: "pre" }}>{top_upvote_tokens_probs[j] ?? ""}</td><td>{(top_downvotes_probs[j] ?? 0).toExponential(1)}</td><td style={{ whiteSpace: "pre" }}>{top_downvote_tokens_probs[j] ?? ""}</td></tr>
                    );
                  })}
                </tbody>
              </table>
